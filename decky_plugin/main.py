...
class Plugin:
    ...
    def _smart_collections_visible(self) -> bool:
        try:
            return (self._settings.get('Collections', 'show_smart_collections', 'true')
                    or 'true') != 'false'
        except Exception:
            return True

    async def get_smart_collections_visible(self):
        return {'success': True, 'enabled': self._smart_collections_visible()}

    async def set_smart_collections_visible(self, enabled: bool):
        try:
            self._settings.set('Collections', 'show_smart_collections',
                               'true' if enabled else 'false')
            logging.info(f"Smart collections visibility set to {bool(enabled)}")
            return {'success': True, 'enabled': bool(enabled)}
        except Exception as e:
            logging.error(f"set_smart_collections_visible error: {e}", exc_info=True)
            return {'success': False, 'message': str(e)}
    ...
    async def get_library_groups(self, mode: str = 'platform'):
        ...
        try:
            ...
            if mode == 'collection':
                ...
                # Smart collections can be hidden from the collections index via
                # a settings toggle — users with filter-heavy libraries can have
                # dozens they never browse.
                show_smart = self._smart_collections_visible()
                ...
                for col in list(self._romm_collections or []) + \
                          ([c for c in (self._romm_smart_collections or [])
                            if show_smart]):
                    name = col.get('name')
                    if not name:
                        continue
                    ...
                    if col.get('is_favorite'):
                        ...
                        kind = 'collection'
                    if kind == 'smart' and not show_smart:
                        continue
                ...
                # section. Keyed by their opaque base64 id — the same key the
                # engine's CollectionSyncManager uses, so they auto-sync like
